...
for idx, cat_col in enumerate(
        categorical_columns):  # converting data to numbers // #cat_col:column(attribute) to be itterated
    row, col = idx // 2, idx % 2
    sns.countplot(x=cat_col, data=train_df, hue='Loan_Status',
                  ax=axes[row, col])  # giving parameters to Bar Chart graph
...
# print(train_df[numerical_columns].describe()) #getting data averages (mean,standard dev.,etc...)
...
#print("poly SVM Precision=", metrics.precision_score(y_test, y_pred))
#print("poly SVM Recall=", metrics.recall_score(y_test, y_pred))
...
# print('p_pred: ', p_prep, sep='\n', end='\n\n')
# print('y_pred:', y_prep, end='\n\n')
print('score_:', score, end='\n\n')
#print('conf_m: ', conf_a, sep='\n', end='\n\n')
#print('end logistic')


# TODO Decision tree:-
#deleted the decision tree functions and replaced them with these 4 lines
